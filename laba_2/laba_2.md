...
(defun my-reverse (lst &optional (acc nil))
  (if (null lst) acc (my-reverse (cdr lst) (cons (car lst) acc))))

(defun nest-from-reversed (rev &optional (acc nil))
  (cond
    ((null rev) acc)
    ((null acc) (nest-from-reversed (cdr rev) (list (car rev))))
    (t          (nest-from-reversed (cdr rev) (list acc (car rev))))))

(defun reverse-and-nest-head (lst)
  (nest-from-reversed (my-reverse lst)))
```
...
```lisp
(defun repeat-n (x n)
  (if (<= n 0) nil (cons x (repeat-n x (1- n)))))

(defun duplicate-elements (lst n)
  (if (null lst)
      nil
      (append (repeat-n (car lst) n)
